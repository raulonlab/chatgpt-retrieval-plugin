...
class Config(object):
    ...
    def get_nerd_config(self, nerd_name: Optional[str] = None, ignore_nerd: Optional[bool] = False):
        config = Config.environment_instance()

        nerd_name = nerd_name or config.get_active_nerd() or None
        if not nerd_name and not ignore_nerd:
            raise ValueError("No nerd name provided")
        
        default_base_path = Path(__file__).parent
        default_config = self.read_nerd_config(default_base_path)

        if nerd_name:
            active_nerd_base_path = config.get_nerd_base_path(nerd_name=nerd_name)
            active_nerd_config = self.read_nerd_config(active_nerd_base_path)

            merged_nerd_config = self.merge_config(default_config, active_nerd_config)
            merged_nerd_config["_nerd_base_path"] = str(active_nerd_base_path)
        else:
            merged_nerd_config = default_config
            merged_nerd_config["_nerd_base_path"] = str(default_base_path)
